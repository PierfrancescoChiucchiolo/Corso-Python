...
sistema a X fasi dove x è il numero dei partecipanti il cui scopo e:
loggare, controllare i dati, gioco, classifica e un logout con possibilità di ripetizione


applicazione per quesiti matematici: login / azione / classifica / logout
piercarlo / pierfrancesco / lorenzo / piercarlo

login serve interazione e poi dizionari di user
azione serve un set di domande ad estrazione randomica
classifica serve dizionario con punteggio
logout


gioco(n = 5) numero dei giochi da fare, restituisce un set
gioco.domande() return insieme di domande e risposte (anche con difficoltà diversa)
gioco.punteggio() inizializzato a 0, chiamabile anche se non è concluso
gioco.fine() return del punteggio


configurazione e coordinamento
gruppo con lorenzo e pier carlo
...
class Gioco:
    ...
    def domande(self, n = 5):
        ...
        for indice in range(0, n):
            random_num = random.randint(0, len(copia_lista_domande) -1)
